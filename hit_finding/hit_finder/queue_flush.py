...
def argparser():
    ...
    parser.add_argument("-ps", "--pattern_start", default = 0,
                        help="first image number e.g. 00000 can be given as 0")
    parser.add_argument("-pe", "--pattern_end", default = 25600,
    ...
    parser.add_argument("-dm","--d_min_cutoff", type=int, default = None,
...
if args.visit_directory: 
   ...
   if args.run_number:
      ...
      run_file = [fil for fil in os.listdir(input_directory) if fil.endswith('00000.cbf')]
      #run_number = int(run_file[0].remove(chip_name).remove('_00000.cbf'))
else:
...
print pattern
output_directory = os.path.join('/dls/i24/data/2018/mx19458-1', 'processing', 'image_analysis')
...
pattern_start = 0
pattern_end =  ca.caget(pv.pilat_numimages)
timeout_first = 3600
timeout = 3600
results_seen = 0
spot_cutoff = 15
...
#results = [ False ] * (pattern_end - pattern_start)

#with open('%s.out'%(chip_name), 'w') as out_file:
#with open(os.path.join(output_directory,'.out'), 'w') as out_file:
#    print('file made')
#    while True:
...
#       results_seen += 1
#       print(results_seen)
#       if int(results_seen) >= (int(pattern_end)-1):
#          break
...
# 
#
#print(results_seen - 1, 'out of ', pattern_end, 'images processed')
